...
print("limit: 1 retry, file size < 50 lines")
...
for tp in rnd_files:
    ...
    patience_config = custom_cost_correction_config(error_limit=1) # adjust this

    start_time = time.time()
    err_trees = patience_parse(tokens, python_productions, init_items_python, error_config=patience_config, hard_limit=0, start_token="file_input")
    end_time = time.time()
